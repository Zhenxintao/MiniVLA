...
#     return resize_size
def get_image_resize_size(cfg):
    if cfg.model_family in ["prismatic", "openvla", "trt_vla"]:
        return 224
    else:
        raise ValueError("Unexpected `model_family` found in config.")
...
def get_action(cfg, model, obs, task_label, processor=None):
    ...
    if cfg.model_family == "prismatic":
        ...
        assert action.shape == (ACTION_DIM,)
    elif cfg.model_family == "trt_vla":
        # Reuse the action reasoning process of Prismatic (the upper-level logic remains unchanged, and the underlying backbone has been replaced with the TRT version)
        action = get_prismatic_vla_action(
            model, processor, cfg.pretrained_checkpoint, obs, task_label, cfg.unnorm_key, center_crop=cfg.center_crop
        )
        assert action.shape == (ACTION_DIM,)
