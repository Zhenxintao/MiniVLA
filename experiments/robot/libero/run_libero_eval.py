...
def eval_libero(cfg: GenerateConfig) -> None:
    ...
    if cfg.model_family in ["openvla", "prismatic", "trt_vla"]:
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        family_for_env = "prismatic" if cfg.model_family == "trt_vla" else cfg.model_family
        
        # Initialize LIBERO environment and task description
        # env, task_description = get_libero_env(task, family_for_env, resolution=resize_size)
        env, task_description = get_libero_env(task, family_for_env, resolution=video_resolution)
        # Start episodes
        ...
        for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task)):
            ...
            while t < max_steps + cfg.num_steps_wait:
                try:
                    ...
                    if t < cfg.num_steps_wait:
                        obs, reward, done, info = env.step(get_libero_dummy_action(family_for_env))
                        t += 1
                        continue

                    # Get the original image (not resized) for video saving
                    video_img = get_libero_image(obs, resize_size=video_resolution)
            ...
            # )
            save_rollout_video(
                video_images, total_episodes, success=done, task_description=task_description, log_file=log_file
            )
