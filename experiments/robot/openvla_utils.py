...
from .trt_backbone import TRTHttpVisionBackbone
...
def get_prismatic_vla(cfg):
    ...
    for p in vla.parameters():
        assert p.dtype == torch.float32, f"Loaded VLM parameter not in full precision: {p.dtype}"

    print("[DEBUG] original vision backbone:", type(vla.vision_backbone), getattr(vla.vision_backbone, "identifier", None))

    # 2) Vision backbone
    # Here out_num_patches / out_embed_dim are aligned with your TRT vision engine output
    trt_vis = TRTHttpVisionBackbone.from_existing(
        vla.vision_backbone,
        base_url=os.getenv("TRTVISION_BASE_URL", "http://127.0.0.1:8000"),
        out_num_patches=256,
        out_embed_dim=2176,
    )
    for p in vla.vision_backbone.parameters():
        p.requires_grad = False
    vla.vision_backbone = trt_vis
    print("[DEBUG] replaced vision backbone:", type(vla.vision_backbone), vla.vision_backbone.identifier)

    # 3) LLM backbone
    # llm_base_url = os.getenv("TRTLLM_BASE_URL", "http://127.0.0.1:8810")
    # vla.llm_backbone = TRTHttpLLMBackbone.from_existing(
    #     vla.llm_backbone,
    #     base_url=llm_base_url,
    #     tok_dir_path="/home/cody/openvla-mini/export/qwen25-0_5b-trtllm/tokenizer",
    # )
    # print("[DEBUG] prompt_builder fn (class or factory):", vla.llm_backbone.prompt_builder_fn)
    # print("[DEBUG] replaced LLM backbone -> TRTHttpLLMBackbone", llm_base_url)

    def _get_prompt_builder():
        pb = vla.llm_backbone.prompt_builder_fn
        try:
            return pb()   
        except TypeError:
            return pb    

    if not callable(getattr(vla, "get_prompt_builder", None)):
        vla.get_prompt_builder = _get_prompt_builder
    else:
        try:
            _test = vla.get_prompt_builder()
            if hasattr(_test, "__class__") and isinstance(_test, type):
                vla.get_prompt_builder = _get_prompt_builder
        except Exception:
            vla.get_prompt_builder = _get_prompt_builder
    ...
    print("### USING get_prismatic_vla (HTTP LLM + HTTP Vision, prompt_builder fixed) ###")
    return vla
